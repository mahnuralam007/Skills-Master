...
- The user asks to generate a skill from a set of instructions.
- The user wants to standardize a workflow into a skill.

## Workflow
1.  **Analyze Request**: Identify the skill name (gerund), description (3rd person), and core logic.
2.  **Generate Structure**: Plan the folder structure (`scripts/`, `examples/` if needed).
3.  **Draft SKILL.md**: Write the content following the "Writing Principles" below.
4.  **Output**: Create the directories and files effectively "installing" the skill.
...
### 5. Output Template
When asked to create a skill, output the result in this format or write it directly to the `.agent/skills/` directory:
